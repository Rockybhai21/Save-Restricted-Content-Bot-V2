   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await message.reply_text("You are Banned. Contact -- @Movie_Pirates_x")
            return 1
      except UserNotParticipant:
        caption = f"Join our channel to use the bot"
        await message.reply_photo(photo="https://envs.sh/g_q.jpg",caption=caption, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{url}")]]))
        return 1
      except Exception:
...
async def progress_callback(current, total, progress_message):
    ...
    f"╰──────────────────╯\n\n"
    f"**__Powered by Movie_Pirates_x__**"
        )
